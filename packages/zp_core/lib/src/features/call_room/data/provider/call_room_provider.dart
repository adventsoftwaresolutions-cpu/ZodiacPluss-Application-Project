...
/// Must be overridden at the app level with a concrete [CallRoomRepository].
///
/// Example (in zp_expert):
/// ```dart
/// final callRoomRepositoryProvider = Provider<CallRoomRepository>((ref) {
///   return ApiCallRoomRepository(api: ..., sessions: ...);
/// });
/// ```
final Provider<CallRoomRepository> callRoomRepositoryProvider =
    Provider<CallRoomRepository>((Ref ref) {
  throw UnimplementedError(
    'callRoomRepositoryProvider must be overridden at the app level.',
  );
});

/// Tracks the room ID of the currently active call, if any.
final StateProvider<String?> activeCallRoomIdProvider =
    StateProvider<String?>((Ref ref) => null);

/// The most recent consultation event received from the backend event stream.
final StateProvider<ConsultationEvent?> latestConsultationEventProvider =
    StateProvider<ConsultationEvent?>((Ref ref) => null);

/// How frequently to poll for active consultation rooms as a fallback.
final Provider<Duration> incomingCallPollIntervalProvider =
    Provider<Duration>((Ref ref) => const Duration(seconds: 2));

/// Grace period before ending a call when the remote participant disconnects.
final Provider<Duration> remoteClientDisconnectGraceProvider =
    Provider<Duration>((Ref ref) => const Duration(seconds: 5));

/// Controller that manages the list of incoming / active consultation rooms.
...
/// Controller that manages the lifecycle of a single call session.
///
/// The consuming app is responsible for providing the [CallRoomRepository]
/// concrete implementation via [callRoomRepositoryProvider].
...
class CallSessionController
  ...
  Future<CallSessionState> build(String roomId) async {
    ...
    // AsyncNotifier initialization may not synchronously mutate another
    // provider. The old expert-only controller happened to cross this boundary
    // while awaiting the expert profile; keep it explicit now that the shared
    // controller has no app-specific profile dependency.
