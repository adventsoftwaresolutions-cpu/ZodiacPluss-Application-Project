...
class KundaliChartsBody extends ConsumerWidget {
  const KundaliChartsBody({required this.birthData, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final query = KundaliChartQuery(
      birthData: birthData,
      chart: selection,
    );
    final chart = ref.watch(kundaliChartProvider(query));
      ...
      child: AnimatedSwitcher(
        ...
        child: chart.when(
          ...
          error: (error, stackTrace) => _ChartError(
            ...
            onRetry: () => ref.invalidate(kundaliChartProvider(query)),
          ),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Row(
      children: KundaliChartStyle.values.map((style) {
        return Expanded(
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: KundaliSpacing.xs),
            child: _ChoicePill(
              label: style.label,
...
class _DivisionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        height: 60,
        padding: const EdgeInsets.symmetric(horizontal: KundaliSpacing.sm),
        decoration: BoxDecoration(
...
class _ChoicePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      borderRadius: BorderRadius.circular(KundaliRadius.xl),
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.symmetric(
          horizontal: KundaliSpacing.sm,
          vertical: 10,
        ),
        decoration: BoxDecoration(
          color: selected
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        const SizedBox(height: KundaliSpacing.xs),
        Text(
          subtitle,
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
                color: Theme.of(context)
