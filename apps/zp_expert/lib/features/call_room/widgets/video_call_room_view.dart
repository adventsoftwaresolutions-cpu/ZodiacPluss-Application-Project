...
class VideoCallRoomView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      fit: StackFit.expand,
      children: <Widget>[
        _RemoteVideoPlaceholder(session: session),
        Positioned(
          left: 18,
          right: 18,
        ...
        if (session.phase != CallSessionPhase.ended)
          Positioned(
            ...
            child: _LocalVideoPreview(isVideoOn: session.isVideoOn),
          ),
        Positioned(
          left: 16,
          right: 16,
...
class _RemoteVideoPlaceholder extends StatelessWidget {
  const _RemoteVideoPlaceholder({required this.session});
...
class _LocalVideoPreview extends StatelessWidget {
  const _LocalVideoPreview({required this.isVideoOn});
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                Theme.of(context).colorScheme.onPrimary.withValues(alpha: .5),
          ),
        ),
